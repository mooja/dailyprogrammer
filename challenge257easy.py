...
# Daily Programmer Challenge 257 Easy
#
...
Record = namedtuple('Record', ['name', 'birth', 'death'])
...
def parse_records(text):
    ...
    records = []
    for row in reader:
        b_month, b_day, b_year = row['BIRTH DATE'].split()
        birth = date(year=int(b_year), month=months[b_month], day=int(b_day))

        try: 
            d_month, d_day, d_year = row['DEATH DATE'].split()
            death = date(year=int(d_year), month=months[d_month], day=int(d_day))
        except ValueError:
            death = None

        name = row['PRESIDENT']
        record = Record(name, birth, death)
        records.append(record)

    return records

def presidents_alive_on_date(records, date_):
    presidents_alive = []
    for r in records:
        if r.birth > date_:
            continue
        if r.death and r.death < date_:
            continue
        presidents_alive.append(r)

    return presidents_alive

def main(data):
    records = parse_records(data)

    n_presidents_alive = {}
    for year in range(1732, 2016):
        d = date(year=year, month=1, day=1)
        n_presidents_alive[year] = len(presidents_alive_on_date(records, d))

    years = sorted(n_presidents_alive.items(), 
                   key=itemgetter(1), 
                   reverse=True)

    for year, npresidents in years[:1]:
        print(year, npresidents)


DATA = """PRESIDENT, BIRTH DATE, BIRTH PLACE,    DEATH DATE, LOCATION OF DEATH
